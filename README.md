# 주행시험장 관리 시스템
: 주행시험장을 예약하고, 주행시험을 진행할 차량을 대여할 수 있게 도와주는 시스템. 사용자는 시험장 예약 및 차량 대여를 진행할 수 있고, 진행한 시험 결과를 게시물로 등록할 수 있으며, 주행한 경로를 .geojson 파일로 업로드하여 지도 위에서 시각화할 수 있다. 관리자 계정으로 로그인하면 각 시험장 및 차량을 수정하거나 추가 등록할 수 있으며, 사용자에 대한 관리를 진행할 수 있다. 보안과 관련한 기능은 스프링 시큐리티로 구현하였다.
